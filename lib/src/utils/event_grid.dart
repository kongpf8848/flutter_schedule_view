...
class EventDrawProperties {
  ...
  void calculateTopAndHeight(double Function(HourMinute time, {HourMinute minimumTime, double? paddingTop}) topOffsetCalculator) {
    if (!shouldDraw) {
      return;
    }

    top = topOffsetCalculator(HourMinute.fromDateTime(dateTime: start!));
    height = topOffsetCalculator(HourMinute.fromDuration(duration: end!.difference(start!)), minimumTime: HourMinute.min, paddingTop: 0) + 1;
  }
  ...
  Widget createWidget(BuildContext context, DayView dayView,Widget? resizeUpGestureDetector, Widget? resizeDownGestureDetector, FlutterWeekViewEvent event) {
    ...
    if (dayView.dragAndDropOptions != null) {
      if (dayView.dragPredicate == null || dayView.dragPredicate!(event)) {
    ...
    if (resizeUpGestureDetector != null || resizeDownGestureDetector != null) {
      if (dayView.resizePredicate == null || dayView.resizePredicate!(event)) {
        child = Stack(
          alignment: Alignment.center,
          children: [
            Positioned(child: child),
            if(resizeUpGestureDetector!=null)
            Positioned(
              left: 0,
              right: 0,
              top: 0,
              height: 16,
              child: resizeUpGestureDetector,
            ),
            if(resizeDownGestureDetector!=null)
            Positioned(
              left: 0,
              right: 0,
              bottom: 0,
              height: 16,
              child: resizeDownGestureDetector,
            ),
          ],
        );
        return Positioned(
