...
typedef CurrentTimeIndicatorBuilder = Widget? Function(DayViewStyle dayViewStyle, TopOffsetCalculator topOffsetCalculator, HoursColumnStyle hoursColumnStyle, bool isRtl,ValueNotifier<int> currentTimeNotifier);
...
typedef EventPredicate = bool Function(FlutterWeekViewEvent event);
...
abstract class ZoomableHeadersWidget<S extends ZoomableHeaderWidgetStyle, C extends ZoomController> extends StatefulWidget {
  ...
  final EventPredicate? dragPredicate;

  final EventPredicate? resizePredicate;
  ...
  final ValueNotifier<DateTime?>? timeRangeStartNotifier;

  final ValueNotifier<DateTime?>? timeRangeEndNotifier;
  ...
  const ZoomableHeadersWidget({
    ...
    this.timeRangeStartNotifier,
    this.timeRangeEndNotifier
  });
}
...
abstract class ZoomableHeadersWidgetState<W extends ZoomableHeadersWidget> extends State<W> with ZoomControllerListener {
  ...
  double calculateTopOffset(HourMinute time, {HourMinute? minimumTime, double? hourRowHeight, double? paddingTop}) =>
      DefaultBuilders.defaultTopOffsetCalculator(time, minimumTime: minimumTime ?? widget.minimumTime, hourRowHeight: hourRowHeight ?? this.hourRowHeight, paddingTop: paddingTop ?? widget.padding.top);

  /// Given a local position in the widget, calculates its corresponding
  /// HourMinute.
  HourMinute calculateOffsetHourMinute(Offset localOffset) {
      double hourMinutesInHour = (math.max(0,localOffset.dy)) / hourRowHeight;
